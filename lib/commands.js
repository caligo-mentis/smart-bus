...
module.exports = {
  ...
  0x0002: {
    ...
    encode: function(data) {
      return Buffer.from([data.area, data.scene]);
    },
  ...
  0x0003: {
    ...
    encode: function(data) {
      ...
      var buffer = Buffer.allocUnsafe(3 + bytes);
  ...
  0xEFFF: {
    ...
    encode: function(data) {
      ...
      var length = channels.length;
      var bytes = Math.ceil(length / 8);
      var buffer = Buffer.allocUnsafe(2 + areas.length + bytes);
  ...
  0x0005: {
    ...
    encode: function(data) {
      ...
      var channels = data.channels;
      var length = channels.length;
      var buffer = Buffer.allocUnsafe(5 + length);
  ...
  0x0001: {
    ...
    encode: function(data) {
      var channels = data.channels || [];
      var length = channels.length;
      var buffer = Buffer.allocUnsafe(4 + length);
  ...
  0x001A: {
    ...
    encode: function(data) {
      return Buffer.from([data.area, data.sequence]);
    },
  ...
  0xF116: {
    ...
    encode: function(data) {
      return Buffer.from([data.block, data.status ? 1 : 0]);
    }
  },
  ...
  0xE018: {
    ...
    encode: function(data) {
      return Buffer.from([data.switch]);
    },
  ...
  0xE3E0: {
    ...
    encode: function(data) {
      return Buffer.from([data.curtain, data.status]);
    },
  ...
  0xE3E2: {
    ...
    encode: function(data) {
      return Buffer.from([data.curtain]);
    },
  ...
  0xE3E3: {
    parse: function(buffer) {
      ...
      // Duration field (4 bytes total) - in deciseconds (0.1 second units)
      if (buffer.length >= 4) {
        data.duration = buffer.readUInt16BE(2) / 10;
      }
      return data;
    },

    encode: function(data) {
      // Support both 2-byte (legacy) and 4-byte (with duration) formats
      if (data.duration !== undefined) {
        var buffer = Buffer.allocUnsafe(4);
        buffer.writeUInt8(data.curtain, 0);
        buffer.writeUInt8(data.status, 1);
        var duration = data.duration * 10;
        buffer.writeUInt16BE(duration, 2);
        return buffer;
      } else {
        return Buffer.from([data.curtain, data.status]);
      }
  ...
  0xE801: {
    parse: function(buffer) {
      return {
        channel: buffer.readUInt8(0),
        duration: buffer.readUInt16BE(3)
      };
    },
    ...
    encode: function(data) {
      ...
      buffer.writeUInt16BE(data.duration || 0, 3);
      return buffer;
    }
  },
  ...
  0x1945: {
    ...
    encode: function(data) {
      ...
      var buffer = Buffer.allocUnsafe(8 + timerPresent);
      var temperature = data.temperature;
  ...
  0x1646: {
    ...
    encode: function(data) {
      ...
      var buffer = Buffer.allocUnsafe(hasSonic ? 8 : 7);
  ...
  0xE3E8: {
    ...
    encode: function(data) {
      ...
      return Buffer.from([data.channel, temperature | sign]);
    }
  },
  ...
  0x15D0: {
    ...
    encode: function(data) {
      var buffer = Buffer.allocUnsafe(3);
      buffer.writeUInt8(data.area, 0);
      buffer.writeUInt8(data.channel, 1);
      buffer.writeUInt8(data.status ? 0 : 1, 2);
      return buffer;
    }
  },
  ...
  0x6F00: {
    parse: function(buffer) {
      var data = {
        declaredChannels: buffer.readUInt8(1),
        statusMask: buffer.readUInt16BE(2),
        channels: []
      };
      
      // Include subnet if present
      if (buffer.length >= 4) {
        data.subnet = buffer.readUInt8(0);
      }
      
      // Parse 16-bit mask for up to 16 channels
      var mask = data.statusMask;
      var channels = data.channels;
    ...
    encode: function(data) {
      var buffer = Buffer.allocUnsafe(4);
      buffer.writeUInt8(data.subnet || 0, 0);
      buffer.writeUInt8(data.declaredChannels || 16, 1);
  ...
  0xE14F: {
    ...
    encode: function(data) {
      return Buffer.from([data.button]);
    }
  }
};
